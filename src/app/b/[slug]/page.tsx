...
function parseHoursLines(googleHours: string | null | undefined) {
  ...
  return googleHours
    ...
    .map((line) => {
      ...
      return { day: dayIndex >= 0 ? short[dayIndex] : day, hours: hrs, isToday };
    });
}
...
export default async function BusinessMiniSitePage({
  ...
  const shareUrl = profile?.customDomain
    ...
    : `${process.env.NEXT_PUBLIC_APP_URL ?? "https://wehearyou.vercel.app"}/b/${location.slug}`;
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        ...
        {/* Name + rating — below logo, no overlap */}
        <div className="mt-3">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 border-b border-slate-200 pb-5">
          {reviewDestination ? (
            <a href={reviewDestination} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-5 py-2.5 text-sm font-semibold !text-white shadow-sm visited:!text-white hover:!text-white"
              style={{ color: "white" }}>
            ...
            <Link href={`/f/${location.slug}`}
              className="inline-flex rounded-2xl bg-slate-950 px-5 py-2.5 text-sm font-semibold !text-white shadow-sm visited:!text-white hover:!text-white"
              style={{ color: "white" }}>
              Write a review
            </Link>
          )}
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        <div className="grid gap-8 lg:grid-cols-[300px_1fr]">
          ...
          <aside className="space-y-5">
                ...
                {fullAddress && (
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      {mapUrl && (
                        <a href={mapUrl} target="_blank" rel="noreferrer" className="mt-1 inline-block text-xs font-semibold text-indigo-600 hover:underline">
                          Get directions →
                        </a>
                      )}
                    </div>
            ...
            {/* Map — only render iframe when we have an embed key */}
            {mapUrl && (
              <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
...
// Parse "Monday: 9:00 AM – 5:00 PM" style lines from GBP
function parseHoursLines(googleHours: string | null | undefined) {
  if (!googleHours) return [];
  const days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
  const short = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  return googleHours
    ...
    .map((line, i) => {
      ...
      const isToday = dayIndex === todayIndex;
      return { day: dayIndex >= 0 ? short[dayIndex] : day, hours, isToday };
    });
}
...
function ReviewerAvatar({ name, photoUrl }: { name: string; photoUrl?: string | null }) {
  ...
  const color = colors[name.charCodeAt(0) % colors.length];
  return (
    <div className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-semibold ${color}`}>
      {initials || "?"}
    </div>
  );
}
...
export default async function BusinessMiniSitePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const heroImage = profile?.heroImageUrl ?? null;
  const logoUrl = profile?.logoUrl ?? null;
  const totalReviews = publicReviews.length + testimonials.length;
  const displayRating = stats.averageRating !== "0.0" ? stats.averageRating : (location.avgRating?.toFixed(1) ?? null);
  const allReviews = [...publicReviews, ...testimonials];
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {/* ── PROFILE HEADER ── */}
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="relative -mt-14 flex items-end gap-5 sm:-mt-16">
          {/* Logo / avatar */}
          ...
          <div className="pb-2 flex-1 min-w-0">
            <h1 className="truncate text-2xl font-bold text-slate-950 sm:text-3xl">{profile?.headline || location.name}</h1>
            {displayRating && (
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <StarRating rating={parseFloat(displayRating)} />
                <span className="text-sm font-semibold text-slate-700">{displayRating}</span>
        ...
        {/* Action bar */}
        <div className="mt-5 flex flex-wrap items-center gap-3 border-b border-slate-200 pb-5">
          {reviewDestination ? (
            <a
              href={reviewDestination}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-5 py-2.5 text-sm font-semibold !text-white shadow-sm visited:!text-white hover:!text-white"
              style={{ color: "white" }}
            ...
            <Link href={`/f/${location.slug}`} className="inline-flex rounded-2xl bg-slate-950 px-5 py-2.5 text-sm font-semibold !text-white shadow-sm visited:!text-white hover:!text-white" style={{ color: "white" }}>
              Write a review
            </Link>
          )}
          {bookingUrl && (
                  ...
                  <tbody>
                    {hours.map(({ day, hours: h, isToday }) => (
                      <tr key={day} className={isToday ? "font-semibold text-indigo-700" : "text-slate-600"}>
                        <td className="py-1 pr-4 w-10">{day}</td>
                        <td className="py-1">{h || "Closed"}</td>
                        {isToday && <td className="py-1 pl-2 text-xs text-indigo-500">Today</td>}
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="mt-3 overflow-hidden rounded-2xl bg-slate-100">
                  {location.googlePlaceId ? (
                    <iframe
                      ...
                      loading="lazy"
                      allowFullScreen
                      src={`https://www.google.com/maps/embed/v1/place?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_EMBED_KEY ?? ""}&q=place_id:${location.googlePlaceId}`}
                    />
                  ) : (
                    <div className="flex h-44 items-center justify-center bg-slate-100">
                      <a href={mapUrl} target="_blank" rel="noreferrer" className="text-sm font-semibold text-indigo-600">
                        Open in Google Maps →
                      </a>
                    </div>
                  )}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-3 text-sm leading-7 text-slate-600">
                          {truncateReviewBody(review.body, 320)}
                        </p>
                        {review.sourceReviewUrl && (
                          <a href={review.sourceReviewUrl} target="_blank" rel="noreferrer"
                            className="mt-2 inline-block text-xs font-semibold text-indigo-600 hover:underline">
